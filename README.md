...
4.  **직인 자동 삽입**:
    *   Google Drive의 `/견적서` 폴더에 `seal.jpeg`가 있으면 견적서에 자동 삽입됩니다.
    *   직인이 없으면 경고 로그만 남고 출력은 계속됩니다.
5.  **자동 저장 및 관리**:
...
직인을 표시하려면 Google Drive의 루트에 `견적서` 폴더를 만들고 `seal.jpeg` 파일을 업로드하세요.
...
*   `seal-sample.png`, `seal-sample.svg`: 로컬 미리보기용 직인 샘플
